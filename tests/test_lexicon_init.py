...
def test_detect_areas_excludes_lexicon_charter_and_stray_files(vault: Path):
    ...
    assert lexicon_init._detect_areas(vault) == ["acme"]
...
def test_scaffold_direction_never_overwrites_existing_file(vault: Path):
    existing = (vault / "Direction" / "acme.md").read_text(encoding="utf-8")

    created = lexicon_init.scaffold_direction(vault, ["acme"])

    assert created == []
    assert (vault / "Direction" / "acme.md").read_text(encoding="utf-8") == existing


def test_scaffold_direction_creates_missing_area_and_is_idempotent(vault: Path):
    (vault / "Memory" / "health").mkdir(parents=True)

    created = lexicon_init.scaffold_direction(vault, ["acme", "health"])
    ...
    created_again = lexicon_init.scaffold_direction(vault, ["acme", "health"])
