...
3. **Create meeting note** — Path: `Meetings/<Project>/YYYY-MM-DD [Title].md`. Apply `.cursor/rules/summarize.mdc` (all steps). Key points from registries:
   ...
   - **Topics**: use canonical slugs from the topic registry. Add new topics to the registry before using them when possible.
...
- **Missing project** — Ask user which project (check `Metadata/project_registry.md` for valid options when available).
