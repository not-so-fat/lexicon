---
name: lexicon-triage
description: Interactive triage — recap with user, update Memory Current model, clean Ideas queue. Not for meetings. Use when user says "triage <project>", "recap career", or wants to discuss how things are going and update memory.
---
...
| Update `# Current model`, resolve open decisions, rare Direction edits | Distilling meetings (use **lexicon-distill**) |
...
**Distill** writes important statements to `# Evidence`. **Triage** synthesizes into `# Current model` when you agree.

**Ideas disposition:** **Keep** only if the user will **keep editing** the idea file. Otherwise **Promote** knowledge into Memory (then delete the idea) or **Retire** if stale.

## Inputs

- **project** — required (e.g. `company`, `career`)
...
| 3. Memory | Propose `# Current model` / People / snapshots; user approves |
| 4. Ideas | Cluster queue → Promote / Keep / **Retire (delete)**; user approves |
| 5. Log | Append `Metadata/recap/<project>/YYYY-MM.md` |

Switch to **Agent mode** to execute approved writes. Small sessions (one area file + a few ideas) can stay in Agent throughout.
...
2. **Recap (conversation)** — Narrative from recent meetings + Memory + Me.md. Discuss open problems. Wait for user input before writes.

3. **Memory updates** — Propose `# Current model` changes, resolve open decisions, Canon (Direction), People `# Current read`. User approves first.
...
- Update `# Current model` without user approval.
